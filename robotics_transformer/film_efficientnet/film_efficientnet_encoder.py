...
from robotics_transformer.film_efficientnet.film_conditioning_layer import \
    FilmConditioning
...
def EfficientNet(width_coefficient,
        ...
        for j in range(round_repeats(args.pop('repeats'))):
            ...
            if include_film:
                with tf.compat.v1.variable_scope('film_conditioning'):
                    x = FilmConditioning(
                        num_channels=x.shape[-1])(x, context_input)
            b += 1
    ...
    if weights == 'imagenet':
        ...
        weights_path = os.path.join(
            os.path.dirname(__file__), WEIGHTS_PATHS[key])
...
def maybe_restore_with_film(
    ...
    **kwargs,
    ...
    l1 = {_l.name: _l for _l in n1.layers}
    # The layers with the film layers.
    l2 = {_l.name: _l for _l in n2.layers}
