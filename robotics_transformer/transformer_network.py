...
from robotics_transformer.tokenizers import action_tokenizer, image_tokenizer
...
class TransformerNetwork(network.Network):
    ...
    def attention_scores(self) -> list[tf.Tensor]:
        ...
        This is for debugging/visualization purpose.
        """
    ...
    def add_summaries(self, observations: dict[str, tf.Tensor],
        ...
        if debug_summaries:
            ...
            image = tf.expand_dims(
                tf.concat(tf.unstack(image, axis=0)[0:8], 0), 0)
            tf.summary.image(
                'observations/image',
                image,
                    ...
                    for head_idx in range(layer_attention_score.shape[1]):
                        ...
                        if img_tf_ts_concat.shape[1] == image.shape[
                            ...
                            overlay_viz = tf.cast(
                                ...
                                tf.uint8)
                            tf.summary.image(
                                'overlay_attention/layer_{}/head_{}'.format(
                                    l_idx, head_idx),
                                overlay_viz,
                                step=self._train_step_counter,
                                # Single output since we have concatenated images along batch.
