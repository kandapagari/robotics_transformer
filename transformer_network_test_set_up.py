...
from robotics_transformer import sequence_agent, transformer_network
...
def observations_list(training: bool = True) -> list[dict[str, tf.Tensor]]:
    """Lists the different types of observations accepted by the
    transformer."""
    if training:
...
class TransformerNetworkTestUtils(tf.test.TestCase, parameterized.TestCase):
    ...
    def _define_specs(self,
        ...
        self._inference_observation = {
            'image':
                tf.constant(
                    1,
                    shape=[self.inference_batch_size,
                           image_height, image_width, 3],
    ...
    def get_action_logits(self, batch_size: int, value: int,
        ...
        return tf.broadcast_to(
            tf.one_hot(value % vocab_size, vocab_size)[
    ...
    def fake_transformer(
        ...
        for j in range(window_range):
            ...
            for t in range(NUM_IMAGE_TOKENS):
                self.assertAllEqual(
                    ...
                    tf.ones_like(all_tokens[0][image_idx]
                                 [:self.token_embedding_size]),
                    all_tokens[0][image_idx + t][:self.token_embedding_size])
            # if j is not the current step in the window, all action dimensions
            # from previous steps are already inferred and thus can be checked.
            action_dims_range = self.action_inf_idx if j == window_range - \
                1 else self._num_action_tokens
            for t in range(action_dims_range):
                token_idx = action_start_index + t
                action_value = (first_step_idx + j) * \
                    self._num_action_tokens + t
                self.assertAllEqual(
        ...
        output_tokens = tf.concat([
            ...
            axis=1)
        self.action_inf_idx = (self.action_inf_idx +
                               1) % self._num_action_tokens
        attention_scores = []
