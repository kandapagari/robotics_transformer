...
from robotics_transformer.transformer_network_test_set_up import (
    BATCH_SIZE, NAME_TO_INF_OBSERVATIONS, NAME_TO_STATE_SPECS,
    TIME_SEQUENCE_LENGTH, TransformerNetworkTestUtils, observations_list,
    spec_names_list, state_spec_list)
...
class TransformerNetworkTest(TransformerNetworkTestUtils):
    ...
    def testTransformerLogging(self, state_spec, train_observation):
        ...
        network_state = tensor_spec.sample_spec_nest(
            network.state_spec, outer_dims=[BATCH_SIZE])
        _ = network(train_observation, step_type=None,
    ...
    def testTransformerCausality(self, state_spec):
        ...
        def _split_image_and_action_tokens(all_tokens):
            ...
            image_tokens = tf.stack(
                [all_tokens[i:i + tokens_per_image]
                    for i in image_start_indices],
                axis=0)
            action_start_indices = [
                i + tokens_per_image for i in image_start_indices]
            action_tokens = [
                tf.stack([
                    all_tokens[i:i + tokens_per_action] for i in action_start_indices
                ], 0)
            ]
            image_tokens = tf.one_hot(
                image_tokens, network._token_embedding_size)
        ...
        for t in range(time_sequence_length *
            ...
            self.assertAllEqual(
                output_tokens[:t + 1], output_tokens_at_t[:t + 1])
